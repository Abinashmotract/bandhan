...
export const MessengerView = ({
  ...
  }, [activeTab, showMessagesOnly]);

  // Load interests list from new API
  const loadInterestsList = async () => {
    try {
      setLoadingInterests(true);
      const response = await messagingAPI.getInterestsList({
        page: 1,
        limit: 50,
        withMessages: showMessagesOnly,
      });

      if (response.data.success) {
        const users = response.data.data.users || [];
        setInterestsList(users);
      }
    } catch (error) {
      console.error("Error loading interests list:", error);
    } finally {
      setLoadingInterests(false);
    }
  };

  // Load chat rooms from new API
  const loadChatRooms = async () => {
    try {
      setLoadingChatRooms(true);
      const response = await messagingAPI.getChatRooms({
        page: 1,
        limit: 50,
  ...
  // Transform interests list to conversation format
  const transformInterestsList = useMemo(() => {
    return interestsList.map((user) => ({
      id: user._id,
      userId: user._id,
      name: user.name,
      age: user.age,
      ...
      date: formatDate(user.lastMessage?.createdAt),
      hasMessages: !!user.lastMessage,
  ...
  const transformChatRooms = useMemo(() => {
    return chatRooms.map((room) => ({
      id: room._id,
      userId: room.otherUser._id,
      name: room.otherUser.name,
      age: room.otherUser.age,
      height: formatHeight(room.otherUser.height),
      caste: room.otherUser.caste || "N/A",
      location: room.otherUser.location,
      avatar: room.otherUser.profileImage,
      date: formatDate(room.lastMessageAt),
      hasMessages: !!room.lastMessage,
      lastMessage: room.lastMessage?.content,
      isOnline: room.otherUser.isOnline || false,
      mutualInterests: 0,
      status: "accepted",
      customId: room.otherUser.customId,
  ...
  const getCurrentData = () => {
    switch (activeTab) {
      case "acceptances":
        return transformChatRooms;
      case "interests":
        return transformInterestsList;
      case "calls":
        return [];
      default:
        return [];
  ...
  const handleBackFromChat = () => {
    setSelectedProfile(null);
    // Reload data when coming back from chat
    if (activeTab === "interests") {
      loadInterestsList();
    } else if (activeTab === "acceptances") {
      loadChatRooms();
