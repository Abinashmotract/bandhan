...
// Get chat rooms (Acceptances tab)
export const getChatRooms = createAsyncThunk(
  "messaging/getChatRooms",
  async (params, { rejectWithValue }) => {
    try {
...
// Get interests list (Interests tab) - NEW ACTION
export const getInterestsList = createAsyncThunk(
  "messaging/getInterestsList",
  async (params, { rejectWithValue }) => {
    try {
      const response = await messagingAPI.getInterestsList(params);
      return response.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Failed to get interests list"
      );
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(sendMessage.fulfilled, (state, action) => {
        ...
        const message = action.payload.data;
        const userId =
          message.receiver || message.receiverId || message.senderId;
        if (!state.messages[userId]) {
          state.messages[userId] = [];
        }
        state.messages[userId].push(message);
      ...
      // Get interests list - NEW CASE
      .addCase(getInterestsList.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getInterestsList.fulfilled, (state, action) => {
        state.loading = false;
        const payloadData = action.payload?.data || {};
        state.interestsList = payloadData.users || [];
        state.pagination = payloadData.pagination || null;
      })
      .addCase(getInterestsList.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
      ...
      .addCase(getTypingStatus.fulfilled, (state, action) => {
        state.loading = false;
        const { userId, data } = action.payload;
        state.typingUsers[userId] = data.data?.isTyping || false;
      })
      .addCase(getTypingStatus.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
